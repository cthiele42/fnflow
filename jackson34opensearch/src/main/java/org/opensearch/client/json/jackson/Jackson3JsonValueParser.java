...
class Jackson3JsonValueParser {
    ...
    public JsonValue parseValue(JsonParser parser) {
        return switch (parser.currentToken()) {
            ...
            case VALUE_STRING -> DefaultJsonProvider.INSTANCE.createValue(parser.getString());
            case VALUE_NUMBER_FLOAT, VALUE_NUMBER_INT -> switch (parser.getNumberType()) {
                case INT -> DefaultJsonProvider.INSTANCE.createValue(parser.getIntValue());
                case LONG -> DefaultJsonProvider.INSTANCE.createValue(parser.getLongValue());
                case FLOAT, DOUBLE -> DefaultJsonProvider.INSTANCE.createValue(parser.getDoubleValue());
                case BIG_DECIMAL -> DefaultJsonProvider.INSTANCE.createValue(parser.getDecimalValue());
                case BIG_INTEGER -> DefaultJsonProvider.INSTANCE.createValue(parser.getBigIntegerValue());
            };
            default ->
                    throw new JsonParsingException("Unexpected token '" + parser.currentToken() + "'", new Jackson3JsonpLocation(parser));
        };
    }
}