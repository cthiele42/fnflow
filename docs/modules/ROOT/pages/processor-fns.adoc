...
The `Multiple Functions` actually is an Operator. Normally, in the FnFlow DSL (Domain Specific Language), the `|` (pipe) character is used to combine functions, and it means each function's output will be used as the next function's input; please pay attention to the below sample:

----
idExist|idTrim|nameTrim
----

In this sample, `idExist` reads messages from input, processes them, and sends them to the next function in the pipeline (that is, the `idTrim` in this example). The `idTrim` also does the same thing, and so on until the last function in the pipeline.

But the `Multiple Functions` uses the `\+` character to create a fork inside a pipeline. That means between two `|` it is possible to join functions by using `+` and each function of these joined functions will receive the same message separately. Combining this functionality with the <<changeeventemit>> function will help the FnFlow users to create forks inside the pipelines and have multiple output topics; please pay attention to the below sample:

----
idExist+validateEmitter|idTrim+trimEmitter|nameTrim
----

In this sample, `idExist` and `validateEmitter` will receive the same messages from the input topic, process them, and send the result to the next functions in the pipeline.
