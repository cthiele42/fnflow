...
Normally, the combination of `Multiple Functions` with <<changeeventemit>> uses at the end of the pipeline for emitting different parts of input messages to different topics (please check the first example). However, the `Multiple Functions` can be used in more complicated scenarios throughout each part of the pipeline (please refer to the second example).
