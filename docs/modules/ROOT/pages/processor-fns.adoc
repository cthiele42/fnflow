...
Note: In these situations the `Reduce2One` raises an exception:

* If the `matches` element does not exist, or is not an array.
* If the `matches` element contains more than one item, and one of these items does not contain the `score` element, or the `score` element is not numeric.
